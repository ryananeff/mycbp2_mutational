#!/hpc/users/neffr01/old_tools/anaconda2/bin/python
...
def main(argv):
   ...
   try:
      opts, args = getopt.getopt(argv,"hi:o:m:rpl",["ifile=","ofile=","mutrate=", "ref=", "possible=", "log="])
   except getopt.GetoptError:
      print 'extract_driver_mutations.py -i <in.maf> -o <out.tsv> -m <mut_rate.pickle> (-r <ref.fa>) (-p <possible_mutations.pickle>)'
      sys.exit(2)
   for opt, arg in opts:
      if opt == '-h':
   ...
   if not infile or not outfile:
       ...
       return 1
   # run script
   mut_table = driver_gene_caller(infile, ref, possible, mutratefile)
...
def driver_gene_caller(infile, 
    ...
    # maf file loading
    new_mut_rate = pickle.load(open(mutratefile,"rb"))
    ...
    for k in new_S:
        ...
        for pair in s_gene:
            ...
            elif ("Intronic" in s_gene[pair]) & ("Intronic" in r[pair]): ### NEW 8/31/2017 ###
                sites += s_gene[pair]["Intronic"]*r[pair]["Intronic"][0]/float(r[pair]["Intronic"][1])
        obs_sites = 0
        for a,w in muts_per_gene[k].iteritems(): #loop over each pair in muts_per_gene[gene]
            if "Silent" in w:
                obs_sites += w["Silent"]
            if "Intronic" in w: ### NEW 8/31/2017 ###
                obs_sites += w["Intronic"]
        if sites == 0:
    ...
    for gene_name, row in data_out.iterrows():
        ...
        for muttype in sig_muts_keys:
            ...
            if pred_sites/(n_sites*n_participants) > 1:
                logger.critical("Error in p-value calculation, gene %s" % gene_name)
                logger.critical("x = %d, n_sites = %.3f, n_participants = %d, pred_sites = %.3f" % (x,n_sites,n_participants,pred_sites))
            else:
