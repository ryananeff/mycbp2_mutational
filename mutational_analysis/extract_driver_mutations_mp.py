...
def main(argv):
   infile=None
   outfile=None
   ref_file = './data/human_g1k_v37.fa'
   ...
   if not infile or not outfile:
       ...
       return 1
   # run script
   mut_table = driver_gene_caller(infile, ref_file, possible)
...
def calculate_variant_context(gene1, variants, all_3mer_combinations, gene_strand, kmer_muts_updated, ref_file,
                            logger=logger): #using hg19
        ...
        if 'ref' not in globals():
            ...
            logger.debug("init ref")
        if 'es' not in globals():
            global es
            es = EnsemblRelease(75)
            logger.debug("init ensembl")
...
def driver_gene_caller(infile, 
    ...
    logger.info("Loading in variants")
    ...
    gene_strand = dict()
    ensembldb = pd.read_csv("./data/Homo_sapiens.GRCh37.75.gtf.expanded.csv", usecols=["gene_name", "source", "strand"])
    ...
    kmer_muts_vars_updated = dict()
    parallel_nodes = 2
    pool = mp.Pool(processes=parallel_nodes)
    funclist = []
    
    for gene1, variants in varfile.groupby(varfile["Hugo_Symbol"]):
        inputs = (gene1,variants,all_3mer_combinations,gene_strand, kmer_muts_updated, ref_file,)
        f = pool.apply_async(calculate_variant_context, args=inputs, callback=collect_results) 
        funclist.append(f)
    pool.close()
    pool.join()
